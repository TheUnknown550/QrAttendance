...
export function SendQrEmailsModal({ open, onClose, totalAttendees }: Props) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const mutation = useMutation({
    mutationFn: async (values: SendQrEmailsValues) =>
      unwrapResponse<SendQrEmailsResult>(await api.post("/attendees/send-qr-emails", values)),
    onSuccess: () => {
      reset();
    },
  });
  ...
  return (
    <Dialog onClose={onClose} open={open} title="Send QR code emails">
      {mutation.isSuccess ? (
        <div className="space-y-5">
          <div className="rounded-[12px] border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-800">
            Sending QR codes to <strong>{mutation.data?.recipientCount ?? totalAttendees}</strong> attendee
            {(mutation.data?.recipientCount ?? totalAttendees) === 1 ? "" : "s"}. This runs in the background,
            emails may take a few minutes to arrive for large lists.
          </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
          <p className="text-sm text-slate-500">
            Every attendee in your directory will receive their personal check-in QR code by email, along with
            the event details you enter below.
          </p>
          ...
          <div className="flex flex-wrap gap-3">
            <Button
              disabled={mutation.isPending || totalAttendees === 0}
              icon={<Mail className="size-4" />}
              type="submit"
            >
              {mutation.isPending ? "Sending..." : `Send to ${totalAttendees} attendee${totalAttendees === 1 ? "" : "s"}`}
            </Button>
